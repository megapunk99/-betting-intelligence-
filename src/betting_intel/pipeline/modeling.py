...
class ModelingMixin:
    ...
    # ── Multi-League Training & Predictions (removed — NBA only) ────

    def train_multi_league_models(self):
        """Stub — multi-league training removed during cleanup. NBA only."""
        self.results["metadata"]["multi_league_models"] = {}

    def predict_multi_league_games(self) -> list[Dict[str, Any]]:
        """Stub — multi-league predictions removed during cleanup. NBA only."""
        return []

    def _fetch_league_upcoming(self, league) -> Optional[pd.DataFrame]:
        """Stub — multi-league upcoming fetching removed. NBA only."""
        return None
